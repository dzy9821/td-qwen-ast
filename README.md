...
4. **第三段（"特别是下午会不会下雨"） —— 客户端主动结束触发**：
   ...
   - **终态处理**：flush 出的残余音频通过后台 ASR 任务处理。服务端**等待所有后台 ASR 任务完成**后，将 flush 段的识别文本与终态信号（`status: 2`）捆绑在同一帧中发送（若无残余语音则发送纯终态信号）。结果发送完毕后，**服务端不会主动断开 WebSocket 连接**，而是等待客户端主动关闭，以防止提前断连导致结果未送达或状态异常。
